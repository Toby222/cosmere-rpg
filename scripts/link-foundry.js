...
const FOUNDRY_DATA_PATH_VALIDATION_REGEX = new RegExp(`${path.sep}Data$`); // Does the path end in "Data"
const FOUNDRY_EXPECTED_SUBFOLDERS = ['modules', 'systems', 'worlds'];
...
const { dataPath } = await prompts({
    ...
    validate: (value) => {
        if (!FOUNDRY_DATA_PATH_VALIDATION_REGEX.test(value)) {
            return `"${value}" does not look like a valid Foundry data path. Make sure the path ends in "${path.sep}Data".`;
        }
...
console.log(
    `Successfully created a symlink between ${buildFolderPath} <==> ${symlinkPath}.`,
);
